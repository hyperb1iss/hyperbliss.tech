...
export abstract class VectorShape {
  ...
  isFadingOut: boolean
  opacity = 0
  radius = 0 // Represents the size of the shape for interactions
  scale = 1 // Current scale of the shape
  scaleTarget = 1 // Target scale for morphing
  scaleSpeed = 0.02 // Speed at which the shape scales
  ...
  protected baseSize = 45 // Increased from 30
  ...
  constructor() {
    ...
    this.lifespan =
      Math.random() * (this.config.shapeLifespanMax - this.config.shapeLifespanMin) + this.config.shapeLifespanMin
    this.age = 0
  ...
  public update(
    ...
    this.position[1] = ((this.position[1] + height / 2 + buffer) % wrapHeight) - height / 2 - buffer
